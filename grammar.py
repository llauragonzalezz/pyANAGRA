...
class Grammar:
    ...
    def calculate_first_set_token(self, token, recursive_tokens):
        """
        ...
        if token in self.non_terminals and None in self.productions[token]:
            first_set.add(None)
            ...
            if production is not None:
                ...
                for index, token_prod in enumerate(production):
                    ...
                    if token_prod not in recursive_tokens:
                        first_set_tok = self.calculate_first_set_token(token_prod, recursive_tokens | {token_prod})
    ...
    def calculate_first_set(self):
        ...
        """
        first_set = dict()
        for token in self.terminals | self.non_terminals:
            first_set[token] = self.calculate_first_set_token(token, {token})
        return first_set

    def calculate_first_set_sentence_fs(self, elements, first_set):
        """
    ...
    def calculate_first_set_sentence(self, elements):
        """
        Calculates the 'first' set of a given sentence form in the context of a grammar.
        ...
        set
            A set containing the 'first' set of the given sentence form.
        """
        first_set = self.calculate_first_set()
        first_set_sentence = set()
        epsilon = True
        # Iterate over elements of a given sentence form
        for element in elements:
    ...
    def calculate_follow_set(self):
        ...
        while follow_set != old_follow_set:
            ...
            for token in self.non_terminals:
                ...
                for production in self.productions[token]:
                    ...
                    if production is not None:
                        ...
                        for index, token_prod in enumerate(production):
                            if token_prod in self.non_terminals:  # A -> a B b
                                # Calculate Pri(þ)
                                first_set_beta = self.calculate_first_set_sentence(production[index + 1:])
