...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        if sys.argv[1] == "-p":
            ...
            font.setPointSize(16) # TODO: PONER PARA CAMBIAR EL TAMAÑO DE LA LETRA?? Y FUENTE (?)
            self.text_edit.setFont(font)
            self.text_edit.setReadOnly(True)

            self.mostrar_conjunto_primero()
        elif sys.argv[1] == "-s":
    ...
    def mostrar_tabla(self):
        ...
        for fila, columna in tabla.keys():
            ...
            print("hola", tabla[(fila, columna)])
            if tabla[(fila, columna)]:
                if tabla[(fila, columna)][0] is None:
                    item = str(fila) + "  → ε"
                else:
                    item = QTableWidgetItem( "{} → {}".format(fila, " , ".join([str(x) if x is not None else 'ε' for x in tabla[(fila, columna)][0]])))
            else:
                item = ""
            ...
            if len(tupla) == 2:
                ...
                if produccion:
                    ...
                    if produccion[1][0] is None:
                        ...
                        item = QTableWidgetItem("{} → {}".format(produccion[0], " ".join(str(x) for x in produccion[1][0])))
                else:
                    item = ""
